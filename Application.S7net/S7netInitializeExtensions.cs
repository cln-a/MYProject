...
{
    public static class S7netInitializeExtensions
    {
        public static void RegisterVariableType<T>(this IUnityContainer container,
            S7netRegister register, S7netDevice device) where T : IComparable
        {
            if (!string.IsNullOrEmpty(device.DeviceBrand))
            {
                IO.Add(register.RegisterUri.Trim(), new S7netVariable<T>(register, device));
            }
        }
        
        public static void RegisterVariableType(this IUnityContainer container,
            S7netRegister register, S7netDevice device)
        {
