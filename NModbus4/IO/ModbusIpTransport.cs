namespace Modbus.IO
{
    ...
    {
        protected static readonly object _transactionIdLock = new object();
        ...
        internal static byte[] GetMbapHeader(IModbusMessage message)
        {
            ...
            var memory = new Span<byte>(new byte[7]);
            transactionId.AsSpan().CopyTo(memory);
            //memory[2] = 0;
            //memory[3] = 0;
            length.AsSpan().CopyTo(memory.Slice(4));
            memory[6] = message.SlaveAddress;
        ...
        internal override byte[] BuildMessageFrame(IModbusMessage message)
        {
            ...
            var span = new Span<byte>(new byte[header.Length + pdu.Length]); 
            header.AsSpan().CopyTo(span);
            pdu.AsSpan().CopyTo(span.Slice(header.Length)); 
            return span.ToArray(); 
        }
