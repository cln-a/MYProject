namespace Modbus.IO
{
    ...
    {
        internal ModbusAsciiTransport(IStreamResource streamResource) : base(streamResource)
        {
        ...
        internal override bool ChecksumsMatch(IModbusMessage message, byte[] messageFrame)
            => ModbusUtility.CalculateLrc(message.MessageFrame) == messageFrame[messageFrame.Length - 1];

        internal override byte[] ReadRequest() => ReadRequestResponse();

        internal override IModbusMessage ReadResponse<T>() => CreateResponse<T>(ReadRequestResponse());
