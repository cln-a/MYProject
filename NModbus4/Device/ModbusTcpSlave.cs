namespace Modbus.Device
{
    ...
    public class ModbusTcpSlave : ModbusSlave
    {
        ...
        private ModbusTcpSlave(byte unitId, TcpListener tcpListener) : base(unitId, new EmptyTransport())
        {
            _server = tcpListener ?? throw new ArgumentNullException("tcpListener");
        }

        private ModbusTcpSlave(byte unitId, TcpListener tcpListener, double timeInterval) : base(unitId, new EmptyTransport())
        {
            if (tcpListener != null)
            {
                _server = tcpListener;
        ...
        public ReadOnlyCollection<TcpClient> Masters
            => new ReadOnlyCollection<TcpClient>(_masters.Values.Select(mc => mc.TcpClient).ToList());
        ...
        /// </summary>
        public static ModbusTcpSlave CreateTcp(byte unitId, TcpListener tcpListener) => new ModbusTcpSlave(unitId, tcpListener);
        ...
        public static ModbusTcpSlave CreateTcp(byte unitId, TcpListener tcpListener, double pollInterval)
            => new ModbusTcpSlave(unitId, tcpListener, pollInterval);
